...
export class RatingController {
  ...
  @Get('export')
  @ApiOperation({ summary: 'Export ratings to CSV' })
  @ApiQuery({
    name: 'fields',
    required: false,
    description: 'Comma-separated list of fields to export',
    example: 'id,userId,listingId,rating,comment,createdAt',
  })
  @ApiHeader({
  ...
  async export(
    ...
    const result = await this.ratingService.findAll({ page: 1, limit: 999999 });
    ...
    this.csvExportService.exportToCsv(
      ratings,
      `ratings-export-${new Date().toISOString().split('T')[0]}`,
      res,
      selectedFields,
      language,
    );
